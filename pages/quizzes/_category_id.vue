<template>
  <div class="bg-gray-700 h-full pt-10">
    ...
    <section class="h-full flex flex-col justify-between py-10">
      ...
      <ul v-else class="p-3 flex gap-3 flex-col-reverse h-full bg-gray-700">
        <li v-for="(quizz, i) in quizzes" :key="i" @click="openQuizz(quizz)" class="flex gap-1 p-2 items-center border rounded bg-white">
...
export default {
  ...
  methods: {
    ...
    async openQuizz(quizz){
      this.$router.push('/quizz/'+ quizz?.id)
    },
  }
}
</script>
